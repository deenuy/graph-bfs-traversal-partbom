...
function bfs_traversal(data, positionIdx) {
  ...
  for (j = 0; j < data.length; j++) {
    ...
    while (!stack.isEmpty()) {
      ...
      // add to child_bom dict
      child_bom[positionIdx + "." + childIdx] = {
        id: positionIdx + "." + childIdx,
        product_number: tmpChild.child,
      };
...
// Test results
console.log("\nGet 1.1.5 children: \n");
console.log(bom_api["1.1.5"]["children"]);

// export bom-api-results as json
fs.writeFile("bom-api-results.json", JSON.stringify(bom_api), function (err) {
  if (err) throw err;
